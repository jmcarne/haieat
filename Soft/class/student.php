...
class Student
{
    ...
    public function create_student_info($post_data=array()){
       ...
       $observacions= mysqli_real_escape_string($this->conn,trim($post_data['observacions']));

       $sql="INSERT INTO students (student_name, email_address, contact,country,gender, observacions) VALUES ('$student_name', '$email_address', '$contact','$country','$gender','$observacioms')";
        
        $result=  $this->conn->query($sql);
        
           if($result){
           
               $_SESSION['message']="Creat correctament el projecte";
    ...
    public function update_student_info($post_data=array()){
       ...
       $sql="UPDATE students SET student_name='$student_name',email_address='$email_address',contact='$contact',country='$country',gender='$gender',observacions='$observacions' WHERE student_id =$student_id";
     
        $result=  $this->conn->query($sql);
        
           if($result){
               $_SESSION['message']="Actualitzat correctament el projecte";
           }
    ...
    public function delete_student_info_by_id($id){
       ...
       $sql="DELETE FROM  students  WHERE student_id =$student_id";
        $result=  $this->conn->query($sql);
           ...
           if($result){
               $_SESSION['message']="Borrat correctament el projecte";
